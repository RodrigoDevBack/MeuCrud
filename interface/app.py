...
layout = [[sg.InputText(key='Texto', do_not_clear=False, expand_x=True)],
          ...
          [sg.Text(f'Arquivo atual: {nome_arquivo()}', key='arquivo')],
...
while True:
    ...
    if event == 'Solicitar':
        if values[0]:
            criar_arq(values['Texto'])
            guardar_arquivo(values['Texto'])
            window['arquivo'].update(f'Arquivo atual:{nome_arquivo()}')
            window['arq'].update(f'Arquivo {values['Texto']} foi criado com êxito!!!')
        else:
            try:
                escrever(values['Texto'])
    ...
    if event == 'mostrar':
        window['analise'].update(mostrar_arq())
    if event == 'remove':
        try:
            indice = int(values['indicie'])
            apagado = apagar(indice)
        ...
        window['analise'].update(mostrar_arq())
    if event == 'reescrever':
        retornada = guardar_apagado(reescrever=True)
        window['analise'].update(mostrar_arq())
